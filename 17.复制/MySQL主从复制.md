...
MySQL从3.23版本开始提供复制的功能。<u>复制是指将主数据库的DDL和DML操作通过二进制日志传到复制服务器（也叫从库）上，然后在从库上对这些日志重新执行（也叫重做），从而使得从库和主库的数据保持同步</u>。

MySQL支持一台主库同时向多台从库进行复制，从库同时也可以作为其他服务器的主库，实现链状的复制。

MySQL复制的优点主要包括以下3个方面：

1、<u>如果主库出现问题，可以快速切换到从库提供服务</u>；

2、<u>可以在从库上执行查询操作，降低主库的访问压力</u>；

3、<u>可以在从库上执行备份，以避免备份期间影响主库的服务</u>。

**注意：**<u>由于MySQL实现的是**异步的复制**，所以主从库之间存在一定的差距，在从库上进行的查询操作需要考虑到这些数据的差异，一般只有更新不频繁的数据或者对实时性要求不高的数据可以通过从库查询，实时性要求高的数据仍然需要从主数据库获得</u>。
...
<u>一般我们靠增加冗余的方式来解决单点故障</u>，冗余的级别和方式不一。从设备的角度，我们可以对主机的单个组件进行冗余，比如使用多个网卡，我们也可以对整个主机所有的关键部件进行冗余，在更高的级别上，我们可以对整个主机进行冗余，或者对整个IDC机房进行冗余。从组织管理的角度，我们还可以对维护数据库的人员进行冗余。MySQL的主从架构本质上也是增加一个冗余的从节点来提高可用性。
...
4）**基于Paxos/Raft分布式数据一致性协议。**

注：现在NewSQL架构中比较流程的是基于一致性算法的方式。
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCBEE.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCBEF.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCBF0.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC01.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC02.tmp.jpg) 

3、在从库上，设置唯一的SERVER_ID选项（它应该与你在主库上设置的不同）：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC03.tmp.jpg) 

4、在从库上，通过远程连接从主库进行备份。可以使用mysqldump或mydumper。不能使用mysqlpump，因为二进制日志的位置不一致。

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC13.tmp.jpg) 

从另一个从库进行备份时，必须设置-slave-dump选项。

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC14.tmp.jpg) 

5、在从库上，待备份完成后恢复此备份。

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC15.tmp.jpg) 

6、在从库上，恢复备份后，必须执行以下命令：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC16.tmp.jpg) 

mydumper：＜log_file_name＞和＜position＞存储在元数据文件中：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC27.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC28.tmp.jpg) 

2、可以通过执行以下命令来检查复制的状态

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC29.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC2A.tmp.jpg) 

2、在服务器B上，停止从库的运行，并记下Relay_Master_Log_File和Exec_Master_Log_Pos在SHOW SLAVE STATUS\G命令中的日志位置：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC3B.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC3C.tmp.jpg) 

5、在服务器C上，检查SHOW SLAVE STATUS的输出中的Exec_Master_Log_Pos和Until_Log_Pos（两者应该相同）来等待服务器C同步：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC3D.tmp.jpg) 

6、在服务器B上，找出主库状态，启动从库，并确保它在复制：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC4D.tmp.jpg) 

7、在服务器C上，停止从库的运行，执行CHANGE MASTER TO命令，并指向服务器B。必须使用在前面的步骤中获得的日志位置：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC4E.tmp.jpg) 

8、在服务器C上，启动复制并验证从库的状态：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC4F.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC74.tmp.jpg) 

3、在master2上，检查当前二进制日志的坐标。

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC75.tmp.jpg) 

根据上面的信息，你可以从位置为 473 的 server1.000017 文件处开始在master1上复制。

4、从第3步中获取的位置开始，在master1上执行CHANGE MASTER TO命令：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC76.tmp.jpg) 

5、在master1上开启Slave模式：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC77.tmp.jpg) 

6、最后，你可以设置master2为“可读/写”，这样应用程序就可以开始对它写入数据了。

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC87.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC88.tmp.jpg) 

还要在配置文件中进行一些更改：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC89.tmp.jpg) 

2、在server3上，执行CHANGE MASTER TO命令，使其通过名为master-1的通道成为server1的从库。你可以为它取任何名字：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC9A.tmp.jpg) 

3、在server3上，执行CHANGE MASTER TO命令，使其通过名为master-2的通道成为server2的从库：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC9B.tmp.jpg) 

4、为每个通道执行START SLAVE FOR CHANNEL语句：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC9C.tmp.jpg) 

5、通过执行SHOW SLAVE STATUS语句来验证从库状态：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCC9D.tmp.jpg) 

6、要获取特定通道的从库状态，请执行：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCCAD.tmp.jpg) 

7、这是使用performance schema来监控指标的另一种方式：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCCAE.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCCAF.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCCC0.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCCC1.tmp.jpg) 

2、将主库设置为只读，并确保所有从库都能与主库同步。这一点非常重要，因为主库和从库之间不应该有任何数据不一致：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCCC2.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCCC3.tmp.jpg) 

4、重新启动主库。当重新启动主库时，它将以读/写模式开始运行，并开始接受以GTID模式写入：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCCD4.tmp.jpg) 

5、执行CHANGE MASTER TO命令来设置GTID复制：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCCD5.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCCD6.tmp.jpg) 

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCCD7.tmp.jpg) 

## **区别**

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCCE7.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCCE8.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCCE9.tmp.jpg) 
...
Row：基于行级别，记录每一行数据的变化，也就是将每行数据的变化都记录到binlog中，记录得非常仔细，但并不记录原始SQL，**存储event数据，存储日志量大**，但是不能很直接的进行读取；在复制的时候，并不会因为存储过程或触发器造成主从库数据不一致的问题，但是**记录的日志量较statement格式要大得多**。
...
中继日志文件Relay Log的文件格式、内容和二进制日志文件binlog一样，**唯一的区别在于从库上的SQL线程在执行完当前中继日志文件Relay Log中的事件之后，SQL线程会自动删除当前中继日志Relay Log，避免从库上的中继日志文件占用过多的磁盘空间**。
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCCFA.tmp.jpg) 

其中master.info记录的是 I/O线程连接主库的一些参数，主要包括SHOW SLAVE STATUS显示出的以下5列（master.info不止包括以下5项内容，仅说明较重要的5项）。

#### 结构

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCCFB.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCCFC.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD0D.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD0E.tmp.jpg) 

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD0F.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD10.tmp.jpg) 

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD20.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD21.tmp.jpg) 

MySQL默认的复制即是异步的，主库在执行完客户端提交的事务后会立即将结果返给给客户端，并不关心从库是否已经接收并处理，这样就会有一个问题，主如果crash掉了，此时主上已经提交的事务可能并没有传到从上，如果此时，强行将从提升为主，可能导致新主上的数据不完整。

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD22.tmp.jpg) 

### **整体/部分复制**

MySQL复制可是对整个实例进行复制，也可以对实例中的某个库或者是某个表进行复制。

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD33.tmp.jpg) 

### **全同步复制**

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD34.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD35.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD36.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD46.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD47.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD48.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD49.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD5A.tmp.jpg) 

确认插件已激活：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD5B.tmp.jpg) 

2、在主库上，启用半同步复制并调整超时（例如调整为1秒）：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD5C.tmp.jpg) 

3、在从库上，安装rpl_semi_sync_slave插件：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD5D.tmp.jpg) 

4、在从库上，启用半同步复制，并重新启动从库IO线程：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD6E.tmp.jpg) 

5、你可以通过以下方式监控半同步复制的状态。

要查找以半同步连接到主库的客户端数量，请执行：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD6F.tmp.jpg) 

当发生超时且从库赶上来时，主库在异步和半同步复制之间切换。要检查主库在使用的复制类型，请查看Rpl_semi_sync_master_status的状态（on表示半同步，off表示异步）：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD70.tmp.jpg) 

可以使用此方法来验证半同步复制。

\1) 停止从库的运行：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD71.tmp.jpg) 

\2) 在主库上随便执行一条语句：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD81.tmp.jpg) 

你会注意到主库已切换到异步复制，因为即使在1秒之后（rpl_semi_sync_master_timeout的值为1秒），主库也没有收到从库的任何确认信息：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD82.tmp.jpg) 

\3) 启动从库：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD83.tmp.jpg) 

\4) 在主库上，你会注意到主库已切换回半同步复制：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD84.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD85.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD96.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD97.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCD98.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCDA9.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCDAA.tmp.jpg) 

假设你想使用正则表达式来选择表，就可以使用REPLICATE_WILD_DO_TABLE选项：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCDAB.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCDAC.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCDBC.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCDBD.tmp.jpg) 

出现这种情况是因为主机上已将其删除了，对此，可采取从机直接跳过的方式解决，命令如下：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCDBE.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCDBF.tmp.jpg) 

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCDD0.tmp.jpg) 

### **主键重复**

主从数据不一致时，slave上已经有该条记录，但我们又在master上插入了同一条记录，此时就会报错，报错信息如下：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCDD1.tmp.jpg) 

解决方法：在slave上使用命令“desc hcy.t1; ”先查看一下表结构，如下所示：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCDD2.tmp.jpg) 

查看该表字段信息，得到主键的字段名。

接着删除重复的主键，命令如下：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCDD3.tmp.jpg) 

然后开启同步复制功能，命令如下：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCDD4.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCDE4.tmp.jpg) 

解决方法：在master上，用MySQL binlog分析一下出错的binlog日志在干什么，如下所示：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCDE5.tmp.jpg) 

从上面的信息来看，是在更新一条记录。接着，在slave上查找一下更新后的那条记录，应该是不存在的。命令如下：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCDE6.tmp.jpg) 

然后再到master查看，命令如下：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCDF7.tmp.jpg) 

可以看到，这里已经找到了这条记录。

最后把丢失的数据填补到在slave上，命令如下：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCDF8.tmp.jpg) 

完成上述操作后，跳过报错即可，命令如下：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCDF9.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCDFA.tmp.jpg) 

（2）在主库端插入3条记录：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCDFB.tmp.jpg) 

（3）从库端启动复制进程，检查测试的表，发现首先插入的两条记录被跳过了，只执行了第3条插入语句：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCE0C.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCE0D.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCE0E.tmp.jpg) 

relay_log_info_repository = TABLE的工作原理如图2-153所示。

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCE0F.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCE1F.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCE20.tmp.jpg) 

解决方法：找到同步的binlog日志和POS点，然后重新进行同步，这样就可以有新的中继日志了。

下面来看个例子，这里模拟了中继日志损坏的情况，查看到的信息如下：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCE21.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCE22.tmp.jpg) 

重新建立完主从复制以后，就可以查看一下状态信息了，如下所示：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCE33.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCE34.tmp.jpg) 

2、执行CHANGE MASTER TO MASTER_DELAY=命令，并启动从库。假设你想要1小时的延迟，可以设置MASTER_DELAY为3600秒：

![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCE35.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCE36.tmp.jpg) 
...
既然选择了MyISAM，那么也应该要有心理准备。（还存在其他场景，也不推荐MyISAM在复制结构中使用）
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCE47.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCE48.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCE49.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCE4A.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCE5A.tmp.jpg) 
...
![img](file:///C:\Users\大力\AppData\Local\Temp\ksohtml\wpsCE5B.tmp.jpg) 
...
通过脚本查询RDB中所有DB的IP和端口，然后脚本设置外键约束，执行完脚本后显示主备不一致

**问题分析：**

在脚本中应该增加一个判断，即只能读取RDB中主机IP，不能再备机执行写操作。

#### 备份data后恢复失败

**问题描述：**

备份主DB的整个data目录，恢复的时候将data目录直接拷贝到备机对应目录，然后重新启动，此时备机无法恢复

问题分析：

恢复的时候需要删除备机的auto.cnf，里面存储的是serverid，这个会在备机重启的时候自动生成一个，建立主备关系，如果使用原来备机的serverid显然是不可以的。